...
const makeValidator = function <T extends ValidationSchemas>(schemas: T): ValidationHelpers<typeof schemas> {
	const createValidator = function (key: keyof ValidationSchemas) {
		return function (req: Request) {
			...
			const { value, error } = (
				schemas[key] as Joi.BaseSchema<any>
			).validate(req[key]);
